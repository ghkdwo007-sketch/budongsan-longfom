...
| 2026-07-31 | **사본 없는 원본 작업을 기본으로** — `--skip-remux` 안내를 `_v.edl`/원본 파일로 분기 + 0단계 자정넘김 사전검사 · 검증기 클램프 인식 | run_episode, verify_episode, CLAUDE.md | 비블 요청("원본으로 작업했으면"). TC0 사본은 회차당 13GB 를 남의 촬영 폴더에 만든다. 실측 소스의 `tmcd` 는 59.94p 인데도 **TC 는 29.97 기준**(`time_base 1/30000`)이라 EDL 과 단위가 맞고 자정도 안 넘어 실제 TC EDL 로 충분했다. ① `--skip-remux` 인데도 "TC0 사본에 연결"이라고 안내해 그대로 하면 23시간 유령 클립이 재현됐다 ② 검증기가 `make_edl` 의 **의도된 클램프**(마지막 컷이 소스 끝을 1프레임 넘어 당겨짐)를 실패로 처리해 파이프라인을 중단시켰다 |
...
| 2026-08-01 | **오디오 싱크 드리프트 수정** — 컷 경계를 TC 격자로 스냅(`make_edl.snap_keeps`), EDL·오디오가 같은 격자를 쓴다 · 검증기에 "오디오 = EDL 시퀀스 길이" 추가 | make_edl, make_cut_audio, verify_episode | **비블이 프리미어에서 조립하다 잡았다 — 영상 2캠은 맞는데 오디오만 뒤로 갈수록 어긋나고 길이도 234ms 달랐다.** 59.94p→29.97 TC 로 접을 때 `make_edl` 이 컷의 in/out 을 **컷마다 따로 반올림**해 컷당 최대 0.5 TC프레임을 잃고 그게 누적됐다(10컷 100ms → 45컷 234ms, 최대 250ms). 오디오는 같은 `_cut.xml` 을 읽지만 반올림 없이 소스프레임으로 잘라 격자가 달랐다. 경계를 짝수 소스프레임으로 미리 스냅하면 접기가 딱 떨어져 오차가 원천 소멸(실측 최대 어긋남 250ms → **0.6ms**, 컷 지점은 최대 16.7ms 이동하나 누적 없음). 검증기가 이걸 통과시킨 건 오디오를 EDL 레코드 타임라인이 아니라 **스냅 전 컷 합계**와만 비교해서였다 |
| 2026-08-01 | **색보정을 .cube 3D LUT 로 굽는다** (`make_lut.py`) | engine(신규), 핸드오프 | `_grade_preset.json` 의 값은 이 엔진 파라미터라 **Lumetri 슬라이더와 1:1 대응하지 않는다** — 손으로 옮기면 어긋난다. 이 보정은 화이트밸런스 게인만 고정하면 전부 픽셀 단위 색 변환이라(피부 보정도 HSV 조건이지 공간 마스크가 아니다) LUT 로 정확히 담긴다. 65³ 에서 직접 계산 대비 평균 오차 0.13/255. **캠별로 다른 LUT** — 원래 캐스트가 달라 바꿔 쓰면 안 된다. WB 게인을 참고 프레임에서 재 굽으므로 조명이 바뀌면 다시 구워야 한다 |
...
→ **[기본] 사본을 만들지 않는다 — `--skip-remux` + `_v.edl`.** 소스 TC가 자정을 넘지 않으면
원본의 실제 TC 를 그대로 쓰는 `_v.edl` 변형이 원본 파일에 바로 붙는다(사본 0, 원본 수정 0).
`run_episode.py` 0단계가 자정 넘김을 미리 검사해서, 안 되는 소스는 시작 전에 중단한다.
→ **이 모드에서 `_v_tc0.edl` 을 쓰면 안 된다.** TC0 EDL 은 소스 TC 를 0 으로 쓰는데 원본에는
실제 TC(예: `05:59:45;03`)가 박혀 있어 **23시간짜리 유령 클립**이 생기고 컷이 전부 어긋난다(실측).
...
이때만 원본 크기만큼 사본이 생긴다. 프리미어 '클립 수정 > 시간 코드'는 **오프라인 클립에서 비활성**이라
EDL 워크플로에서 쓸 수 없다.
