...
| 2026-07-31 | 맥 실행환경 복구(`.venv-mac` 를 3.12 로 정리, 3.14 중복 트리 8.4GB 제거) + 화이트밸런스 중성면 부족 가드 | apply_grade_still, .venv-mac | `.venv-mac` 안에 3.12/3.14 두 벌이 겹쳐 있었고 `bin/python` 은 3.12 를 가리키는데 설치본은 3.14 쪽에만 있어 numpy 계열(`cut_gate`·`sync_2cam`·`eye_grade`)이 전부 임포트 실패였다. 색보정은 **중성면 픽셀이 0 이면 평균이 NaN 이 되어 새까만 이미지를 조용히 저장**했다 — 100px 미만이면 화이트밸런스를 건너뛰고 원본을 넘긴다(정상 프레임 결과값은 불변) |
...
> **[철회 2026-07-30] 프리미어 캡션 대조로 "온점·쉼표를 지우면 안 된다 / 25자는 상한이지
> 목표가 아니다(중앙 15자)" 두 결론을 냈다가 둘 다 되돌렸다.** 근거로 삼은 캡션이
> **26.23분** 타임라인(=우리 285컷 납품본 26.24분)에 있었는데 비블 최종본은 **13.84분**이다 —
> 완성본이 아니라 납품 직후 중간 상태였다. 20자로 뽑아 보니 화면에서 짧다는 피드백도 받았다.
> **캡션·시퀀스를 근거로 쓸 때는 얹힌 타임라인 길이가 최종본과 맞는지 먼저 볼 것.**
> 텍스트가 99.90% 일치한다고 그 트랙이 최종 상태인 건 아니다.
>
> 자막 길이는 `subtitle_polish` 가 아니라 상류 `make_subtitles.regroup(max_chars=)` 에서
> 정해진다 — 바꾸려면 컷 단계를 다시 돌려야 한다. 상세는 부동산롱폼 프로파일 README '5) 자막'.
