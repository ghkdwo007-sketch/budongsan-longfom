...
| | |
|---|---|
| OS | **macOS (Apple Silicon)** 또는 **Windows / Linux** |
| 영상 편집 | **Adobe Premiere Pro 25.0+** (FCP7 XML 가져오기) |
| 런타임 | **Python 3.10+**, **ffmpeg** (ffprobe 있으면 더 정밀) |
| 자막용 | macOS → **mlx-whisper** / Windows·Linux → **faster-whisper** (엔진이 자동 선택) |
...
# 2) 의존성
brew install ffmpeg            # macOS. Windows는 winget install Gyan.FFmpeg
pip install -r requirements.txt

# 2-1) Windows + NVIDIA GPU 로 빠르게 전사하려면 (선택 — 없으면 CPU로 돕니다)
