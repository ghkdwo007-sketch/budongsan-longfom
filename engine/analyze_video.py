...
def measure(path):
    ...
    # 아래 세 패스 모두 오디오만 본다 → -vn/-sn/-dn 없으면 4K 소스에서 비디오를 통째로 디코딩한다
    AONLY = ["-vn", "-sn", "-dn"]

    # 라우드니스 + 트루피크
    r = run([FFMPEG, "-hide_banner", "-i", path, *AONLY,
    ...
    # 평균 음량
    r2 = run([FFMPEG, "-hide_banner", "-i", path, *AONLY, "-af", "volumedetect", "-f", "null", "-"])
    mv = re.search(r"mean_volume:\s*(-?\d+\.?\d*)", r2.stderr)
    mean_vol = float(mv.group(1)) if mv else None

    # 무음 비율 (-40dB, 0.5초 기준)
    r3 = run([FFMPEG, "-hide_banner", "-nostats", "-i", path, *AONLY,
