...
def path_to_url(path):
    """로컬 경로 → FCP7 XML <pathurl>. 프리미어가 실제로 쓰는 형식에 맞춘다.

    윈도우: D:\\a\\b.mp4  → file://localhost/D:/a/b.mp4
    맥/리눅스: /a/b.mov   → file://localhost/a/b.mov

    구분자를 '/'로 바꾸고 '/'와 ':'는 인코딩하지 않는다. 그냥 quote() 하면 윈도우에서
    '\\'가 %5C, 드라이브 콜론이 %3A 로 인코딩돼 프리미어가 미디어를 못 찾는다(전부 오프라인).
    """
    ...
    if HAS_FFPROBE:
        try:
            ...
            r = run([FFPROBE, "-v", "error", "-show_entries",
                     ...
                     "sample_rate,channels:stream_disposition=attached_pic",
                     "-of", "json", path])
            ...
            def _rate(s, key):
                try:
                    num, den = s.get(key, "0/0").split("/")
                    return round(int(num) / int(den), 3) if int(den) else None
                except Exception:
                    return None

            # 본편 비디오 스트림 고르기. 카메라 파일에는 썸네일/커버가 별도 비디오 스트림으로
            # 붙어 있다(DJI = mjpeg 1280x720 @90000fps). 순회하며 덮어쓰면 그게 이겨버리므로
            # 커버를 걸러내고 '해상도가 가장 큰' 스트림을 본편으로 본다.
            vids, auds = [], []
            for s in d["streams"]:
                if s.get("codec_type") == "video":
                    ...
                    auds.append(s)
            if vids:
                v = max(vids, key=lambda s: int(s.get("width") or 0) * int(s.get("height") or 0))
                info["width"] = int(v["width"]); info["height"] = int(v["height"])
                fps = _rate(v, "r_frame_rate")
                if fps is None or fps > 1000:        # 썸네일류의 비정상값 방어
                    fps = _rate(v, "avg_frame_rate")
                info["fps"] = fps
            if auds:
                a = auds[0]
                info["samplerate"] = int(a.get("sample_rate", 48000))
                info["channels"] = int(a.get("channels", 2))
...
def detect_silence(path):
    """무음 구간 [(start,end), ...] 반환."""
    # -vn/-sn/-dn: 오디오만 필요하다. 없으면 4K HEVC 같은 소스에서 비디오를 통째로
    # 디코딩+널인코딩해 수십 배 느려진다 (DJI 파일은 mjpeg 썸네일 스트림까지 붙어 있음).
    r = run([FFMPEG, "-hide_banner", "-nostats", "-i", path, "-vn", "-sn", "-dn",
...
def measure_loudness(path, which="input"):
    ...
    r = run([FFMPEG, "-hide_banner", "-i", path, "-vn", "-sn", "-dn",
...
def make_clean_audio(path, out_wav, info, extra_filters="", chain=None):
    ...
    if chain is None:
        chain = (f"{extra_filters}" if extra_filters else "") + (
            ...
            "acompressor=threshold=-20dB:ratio=3:attack=5:release=150:makeup=2,"
            f"loudnorm=I={TARGET_LUFS}:TP=-1.5:LRA=11")
    # -map_metadata -1 : 원본 타임코드를 물려받지 않게 한다. 물려받으면 이 WAV의 시작 TC가
    # 카메라 TC(예: 23:50:03;28)가 되어 EDL/프리미어에서 0 기준 정렬이 어긋난다.
    r = run([FFMPEG, "-hide_banner", "-y", "-i", path,
             "-af", chain, "-vn", "-map_metadata", "-1",
